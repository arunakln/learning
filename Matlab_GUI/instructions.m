...
% Last Modified by GUIDE v2.5 13-Jul-2016 00:23:49
...
% if user click start button without entering Name field, it display error
% to user
if (isempty([obj.String]))    
    set(handles.errmsg,'visible','on');
else
    set(handles.errmsg,'visible','off');    
    handles.subjName = char(obj.String);    
    programs(handles); % Open program page
    close(instructions); % Close the Welcome page
end
