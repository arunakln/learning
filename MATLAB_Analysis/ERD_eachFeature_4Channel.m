...
% analysis for 4 channels related to attention.
%-------------------------------------------------------------------------%
...
%      freq_bands = sort(freq_bands,1);
   ...
   filename = strcat('Result/ERD_eachFeature', dtstr, '.xls');
   ...
   % Add the jar files related to XLWrite to java path.
    ...
    d.data{index, 10} = char('FeatureName');
    d.data{index, 11} = char('ChannelLabel');
    d.data{index, 12} = char('FreqFrom');
    d.data{index, 13} = char('FreqTo');
    d.data{index, 14} = char('df');
    d.data{index, 15} = char('chi_sq');
    d.data{index, 16} = char('N1');
    d.data{index, 16} = char('N2');
    d.data{index, 16} = char('N3');
    ...
    for f=1:size(freq_bands, 1)            
        for ch=1: no_of_channels
            ...
            f_erd_easy{f} =erd_easy_all{f,ch} ;
            f_erd_medium{f} =erd_medium_all{f,ch} ;
            f_erd_difficult{f} =erd_hard_all{f,ch} ;
        end
        
        erd_allData = [f_erd_easy{f}'; f_erd_medium{f}';  f_erd_difficult{f}']; 
        erd_groups = [ones(size(f_erd_easy{f}')); 2 * ones(size(f_erd_medium{f}')); 3 * ones(size(f_erd_difficult{f}'))];

        [P,ANOVATAB,STATS] = kruskalwallis(erd_allData, erd_groups, 'off');

        d.data{index, 1} = f;
        d.data{index, 2} = mean(f_erd_easy{f});
        d.data{index, 3} = mean(f_erd_medium{f});
        d.data{index, 4} = mean(f_erd_difficult{f});
        ...
        d.data{index, 10} = featureLabel{f};
        d.data{index, 11} = channelLabel{ch};
        d.data{index, 12} = freq_bands(f,1);
        d.data{index, 13} = freq_bands(f,2);
        
        d.data{index, 14} = ANOVATAB{2,3};
        d.data{index, 15} = ANOVATAB{2,5};
        d.data{index, 16} = STATS.n(1,1);
        d.data{index, 16} = STATS.n(1,2);
        d.data{index, 16} = STATS.n(1,3);        
