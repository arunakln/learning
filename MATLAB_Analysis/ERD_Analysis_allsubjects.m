...
% analysis for all subjects and each feature
%-------------------------------------------------------------------------%
...
%      freq_bands = sort(freq_bands,1);
   ...
   filename = strcat('Result/ERD_Features_allsubjects', dtstr, '.xls');
   ...
   % Add the jar files related to XLWrite to java path.
    ...
    d.data{index, 1} = char('FeatureID');
    d.data{index, 2} = char('ChannelID');
    d.data{index, 3} = char('MeanERDEasy');
    d.data{index, 4} = char('MeanERDMedium');
    d.data{index, 5} = char('MeanERDDifficult');
    d.data{index, 6} = char('EasyMeanRank');
    d.data{index, 7} = char('MediumMeanRank');
    d.data{index, 8} = char('HardMeanRank');
    d.data{index, 9} = char('PValue');
    d.data{index, 10} = char('PStatus');
    d.data{index, 11} = char('FeatureName');
    d.data{index, 12} = char('ChannelLabel');
    d.data{index, 13} = char('FreqFrom');
    d.data{index, 14} = char('FreqTo');
    ...
    for f=1:size(freq_bands, 1)            
        for ch=1: no_of_channels
            for s= 1: size(subject_files, 1)
                erd_easy_all{f,ch} = erd_easy_feature{s,f,ch,:};
                erd_medium_all{f,ch} = erd_medium_feature{s,f,ch,:};
                erd_hard_all{f,ch} = erd_hard_feature{s,f,ch,:};

            end

            erd_allData = [erd_easy_all{f,ch}'; erd_medium_all{f,ch}';  erd_hard_all{f,ch}']; 
            erd_groups = [ones(size(erd_easy_all{f,ch}')); 2 * ones(size(erd_medium_all{f,ch}')); 3 * ones(size(erd_hard_all{f,ch}'))];

            [P,ANOVATAB,STATS] = kruskalwallis(erd_allData, erd_groups, 'off');
            
            d.data{index, 1} = f;
            d.data{index, 2} = ch;
            d.data{index, 3} = mean(erd_easy_all{f,ch});
            d.data{index, 4} = mean(erd_medium_all{f,ch});
            d.data{index, 5} = mean(erd_hard_all{f,ch});
            
            if(size(STATS.meanranks(1,:), 2) >=1)
                d.data{index, 6}= STATS.meanranks(1,1);
            else
                d.data{index, 6} = 0;
            end

            if(size(STATS.meanranks(1,:), 2) >=2)
               d.data{index, 7}= STATS.meanranks(1,2);
            else
               d.data{index, 7} = 0;
            end

            if(size(STATS.meanranks(1,:), 2) >=3)
                d.data{index, 8}= STATS.meanranks(1,3);
            else 
                d.data{index, 8} = 0;
            end
            ...
            if(P <=0.05)
                d.data{index, 10}= char('Yes');
            else
                d.data{index, 10}= char('No');
            end

            d.data{index, 11} = featureLabel{f};
            d.data{index, 12} = channelLabel{ch};
            d.data{index, 13} = freq_bands(f,1);
            d.data{index, 14} = freq_bands(f,2);
