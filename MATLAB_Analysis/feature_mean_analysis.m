...
% Date : 01-Aug-2016
% This script reads the EEG data and compute mean across each subject,each 
% feature and each channel andquestion type (easy, medium and difficult)
...
% 1-yasir, 2 adil, 3 wale, 4 josh, 5 jensen, 6 sumia, 8 steve, 9 clement,
% 10 charles
...
%      freq_bands = sort(freq_bands,1);
                ...
                for t = 1:size(qd_data,1)
                    ...
                    if( max(eye_blink_filter) < (100 * filter_mean))
                        %XF{s,f,ch,q,tmp_cnt,:} = filtfilt(fd_b{f,:},fd_a{f,:},qd_data(t,:)');
                         EXF(s,f,ch,q,t,:) = mean(filtfilt(fd_b{f,:},fd_a{f,:},qd_data(t,:)'));  
   ...
   filename = strcat('Result/Features_mean_', dtstr, '.xls');
   ...
   % Add the jar files related to XLWrite to java path.
    ...
    d.data{index, 6} = char('DifficultMean');
    d.data{index, 7} = char('FeatureName');
    d.data{index, 8} = char('ChannelLabel');
    d.data{index, 9} = char('FreqFrom');
    d.data{index, 10} = char('FreqTo');
    ...
    for s= 1: size(subject_files, 1)          
        for f=1:size(freq_bands, 1) 
            for ch=1:no_of_channels                    
                ...
                d.data{index, 7} = featureLabel{f};
                d.data{index, 8} = channelLabel{ch};
                d.data{index, 9} = freq_bands(f,1);
                d.data{index, 10} = freq_bands(f,2);
